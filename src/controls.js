...
// intuitive units (ms / Hz) and converted to samples for the simulation/SODA.
...
const GROUPS = [
  ...
  },
  {
    ...
    items: [
      ...
      { key: 'lag', label: 'lag between states', type: 'num', def: 120, min: 10, max: 800, step: 10, unit: 'ms' },
      ...
      { key: 'breadth', label: 'breadth (extent)', type: 'num', def: 200, min: 10, max: 1000, step: 10, unit: 'ms', perState: true },
      { key: 'nEvents', label: 'number of events', type: 'int', def: 8, min: 1, max: 50, step: 1 },
  ...
  },
  {
    ...
    items: [
      ...
      { key: 'invert', label: 'invert slopes (×−1)', type: 'check', def: true },
      { key: 'normalize', label: 'normalize by class mean', type: 'check', def: true },
    ],
  },
  {
    title: 'Display',
    items: [
      { key: 'showPerm', label: 'show permutation null band', type: 'check', def: true },
      { key: 'clipZero', label: 'clip probabilities ≥ 0', type: 'check', def: true },
      { key: 'clipOne', label: 'clip probabilities ≤ 1', type: 'check', def: false },
    ],
  },
];

// Presets reproducing Kern et al. Figure 6 (right, "SODA examples"). Each is a
...
// SODA wants slow, overlapping reactivations, so the base uses a broad breadth.
const FIG6_BASE = {
  nStates: 5, sfreq: 100, duration: 12, noise: 0.06, baseline: 0,
  lag: 120, magnitudeModifier: 1, magnitude: 1, breadth: 240, nEvents: 10,
  jitter: 0, sequence: '0,1,2,3,4', reactivationOrder: '',
};
const PRESETS = [
  {
    id: 'A', label: 'A · Largely overlapping replay',
    desc: 'Full sequence with broad, overlapping reactivations → pronounced onset and offset periods.',
    values: { ...FIG6_BASE },
  },
  {
    id: 'B', label: 'B · Only first & last replayed',
    desc: 'Only the beginning and end of the sequence reactivate → slope barely distinguishable from full replay.',
    values: { ...FIG6_BASE },
    perState: { magnitude: [1, 0, 0, 0, 1] },
  },
  {
    id: 'C', label: 'C · Single reactivation of first item',
    desc: 'Only the first item reactivates → a singular onset period.',
    values: { ...FIG6_BASE },
    perState: { magnitude: [1, 0, 0, 0, 0] },
  },
  {
    id: 'D', label: 'D · Single reactivation of last item',
    desc: 'Only the last item reactivates → a singular offset period.',
    values: { ...FIG6_BASE },
    perState: { magnitude: [0, 0, 0, 0, 1] },
  },
  {
    id: 'E', label: 'E · Mixed-up reactivations',
    desc: 'Individual reactivations occur in a scrambled order → onset/offset still observable.',
    values: { ...FIG6_BASE, reactivationOrder: '0,3,2,1,4' },
  },
  {
    id: 'F', label: 'F · Little overlap',
    desc: 'Narrow, barely overlapping reactivations → superposition of first/last single reactivations dominates the slope.',
    values: { ...FIG6_BASE, breadth: 60, lag: 160 },
  },
];
...
export function initControls(container, onChange) {
  ...
  function getParams() {
    ...
    const simParams = {
      ...
      normalize: false, // SODA normalisation is applied in soda.js, not the sim
    ...
    const sodaParams = {
      ...
      showPerm: checked('showPerm'),
    };

    return { simParams, sodaParams };
  }
