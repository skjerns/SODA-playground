...
expect(!!plotCalls['plot-seq'], 'SODA slope plot drawn');
...
const slope = seqTraces.find((t) => t.name === 'SODA slope');
expect(!!slope && slope.y.length > 2, `SODA slope curve present (len ${slope ? slope.y.length : 0})`);

const statusText = byId['status'].textContent || '';
expect(/events/.test(statusText), `status line: "${statusText}"`);
