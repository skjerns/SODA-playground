...
expect(p.sodaParams.nShuf === 100 && p.sodaParams.invert === true, 'soda defaults present');
expect(p.simParams.lag === 12, `lag 120ms -> ${p.simParams.lag} samples @100Hz`);
...
// 5) presets: B = only first & last item replayed (per-state magnitude)
const presetEl = container.find((e) => e.tag === 'select' && e.id === 'ctl-preset');
presetEl.value = 'B';
presetEl.dispatch('change');
p = controls.getParams();
expect(JSON.stringify(p.simParams.magnitude.perState) === '[1,0,0,0,1]', `B: per-state magnitude first&last (${JSON.stringify(p.simParams.magnitude.perState)})`);
expect(p.simParams.oscillations.length === 0, 'B: preset cleared oscillations');

// 6) preset D = single reactivation of last item
presetEl.value = 'D';
presetEl.dispatch('change');
p = controls.getParams();
expect(JSON.stringify(p.simParams.magnitude.perState) === '[0,0,0,0,1]', `D: per-state magnitude last only (${JSON.stringify(p.simParams.magnitude.perState)})`);

// 6b) preset E = scrambled reactivation order
presetEl.value = 'E';
presetEl.dispatch('change');
p = controls.getParams();
expect(JSON.stringify(p.simParams.reactivationOrder) === '[0,3,2,1,4]', `E: scrambled reactivation order (${JSON.stringify(p.simParams.reactivationOrder)})`);
